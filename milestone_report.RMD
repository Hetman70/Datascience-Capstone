...
subtitle: "Exploratory Analysis for Predictive Text Modelling"
...
This report consist of information related to [text datasets](https://d396qusza40orc.cloudfront.net/dsscapstone/dataset/Coursera-SwiftKey.zip) explains exploratory analysis and my goals for the eventual app and algorithm. 
...
filePathSep <- "/"
fileNameSep <- "."
swiftKeyDirectory <- "~/workingDirectory/data/Coursera-SwiftKey"
...
1. Based on information from [Getting the Data Section](#getting-the-data) and compatibly with our hardware capacity, it was decided to sample 1% of the dataset.
```{r makeSampleFiles}
makeSampleFiles()
```
...
wordcloud(termDocumentMatrix6$Terms, termDocumentMatrix6$Count, 
          scale=c(8,1),random.order = FALSE, rot.per = 0.35,
          max.words = 170, colors = brewer.pal(8, "Dark2"))
```
